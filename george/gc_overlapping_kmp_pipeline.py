...
def kmp_valid_parsing(network_file, output_path, clustering, min_k_core):
  command = 'python3 ~/repos/ERNIE_Plus/Illinois/clustering/eleanor/code/parsing_clusters_strict.py -e ' + network_file + ' -o ./' + output_path + ' -c ./' + clustering  + ' -k ' + str(min_k_core) + ' -p 0' 
  os.system(command)
...
def IKC(network_file, k):
  command = 'python3 modified_IKC.py -e ' + network_file + ' -o ./experiment_0/IKC_' + str(k) + '.clustering -k ' + str(k)
  os.system(command)
