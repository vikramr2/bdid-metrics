...
def main(max_cores: int = 8):
    ...
    print(
        f"Using {max_cores} CPU cores ({max_cores+2} including logger and csv_writer)..."
    )
    logger.log(
        logging.INFO,
        f"Using {max_cores} CPU cores ({max_cores+2} including logger and csv_writer)...",
    ...
    for _ in range(max_cores):
        worker = Worker(
            nodes_to_calc, calculated_tuples, nodes_read, df_edges, df_edges_clustered, log_queue
        )
