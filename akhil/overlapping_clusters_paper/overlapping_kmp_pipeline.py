...
def main(
    ...
    if include_markers:
        ...
        original_marker_nodes, original_marker_mapping = parse_marker_file(
            "original_markers.csv"
        )
...
def cluster_stats(
    ...
    if include_markers:
        candidate_node_coverage, candidate_edge_coverage = get_coverage_node_list(
    ...
    if save_outputs and include_markers:
        ...
        f.write(
            ...
            + str(edge_coverage)
            + ","
            + str(candidate_node_coverage)
            + ","
            + str(candidate_edge_coverage)
            + "\n"
        )
        f.close()
    elif save_outputs:
    ...
    if candidate_criterion == "random":
        ...
        if rank_type == "percent":
            ...
            candidates = [
                ...
                if v >= bdegree_cutoff and v <= tdegree_cutoff
            ]
...
def kmp_valid_parsing(network_file, output_path, clustering, min_k_core):
    command = (
        "pipenv run python3 ~/ERNIE_Plus/Illinois/clustering/eleanor/code/parsing_clusters_strict.py -e "
        + network_file
        + " -o ./"
        + output_path
        + " -c ./"
...
def IKC(network_file, k):
    command = (
        "pipenv run time python3 modified_IKC.py -e "
        + network_file
        + " -o ./experiment_0/IKC_"
        + str(k)
        + ".clustering -k "
